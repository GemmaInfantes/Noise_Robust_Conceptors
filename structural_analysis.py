...
parser.add_argument("--trials_noise", type=int, default=1) #number of trails, for different seeds
parser.add_argument("--trials_esn", type=int, default=1) #number of trails, for different seeds
...
parser.add_argument("--a_in", type=int, default=1) #conceptor aperture initial
parser.add_argument("--a_fin", type=float, default=100) # aperture final
parser.add_argument("--a_step", type=float, default=1) # aperture steps
...
#parameters for the noise and aperture scan
...
#storing the mean SSI for each aperture and mean SSi for each noise level
mxcorrp_noi=np.empty((len(aa),len(k)),dtype=float)
mxcorrp_noi_C_noi=np.empty((len(aa),len(k)),dtype=float)
mxcorrp_noi_C_ctc_m=np.empty((len(aa),len(k)),dtype=float)
mxcorrp_noi_C_avg=np.empty((len(aa),len(k)),dtype=float)

mssi_noi=np.empty((1,len(aa)),dtype=float)
mssi_noi_C_noi=np.empty((1,len(aa)),dtype=float)
mssi_noi_C_ctc_m=np.empty((1,len(aa)),dtype=float)
mssi_noi_C_avg=np.empty((1,len(aa)),dtype=float)


sssi_noi=np.empty((1,len(aa)),dtype=float)
sssi_noi_C_noi=np.empty((1,len(aa)),dtype=float)
sssi_noi_C_ctc_m=np.empty((1,len(aa)),dtype=float)
sssi_noi_C_avg=np.empty((1,len(aa)),dtype=float)
...
for a_idx, a in enumerate(aa):
    ...
    mxcorrp_noi[a_idx,:]=np.mean(xcorrp_noi,axis=1)
    mxcorrp_noi_C_noi[a_idx,:]=np.mean(xcorrp_noi_C_noi,axis=1)
    mxcorrp_noi_C_ctc_m[a_idx,:]=np.mean(xcorrp_noi_C_ctc_m,axis=1)
    mxcorrp_noi_C_avg[a_idx,:]=np.mean(xcorrp_noi_C_avg,axis=1)

mssi_noi=np.mean(mxcorrp_noi,axis=1)
mssi_noi_C_noi=np.mean(mxcorrp_noi_C_noi,axis=1)
mssi_noi_C_ctc_m=np.mean(mxcorrp_noi_C_noi,axis=1)
mssi_noi_C_avg=np.mean(mxcorrp_noi_C_avg,axis=1)

sssi_noi=np.std(mxcorrp_noi,axis=1)
sssi_noi_C_noi=np.std(mxcorrp_noi_C_noi,axis=1)
sssi_noi_C_ctc_m=np.std(mxcorrp_noi_C_noi,axis=1)
sssi_noi_C_avg=np.std(mxcorrp_noi_C_avg,axis=1)
...
plt.errorbar(
    aa, mssi_noi_C_noi, yerr=sssi_noi_C_noi,
...
plt.errorbar(
    aa, mssi_noi_C_ctc_m, yerr=sssi_noi_C_ctc_m,
...
plt.xlabel("Aperture", size=20)
plt.ylabel("Mean SSI",size=19)
...
if corr is True:
    ...
    c='uncorrelated'
plt.savefig(
    f"plots/PCAxcorr_CTCm_ol_N{N}_m{m}_trials{trials}_noisestep{args.noise_steps}_maxnoise{args.noise_max}_a{a}_steps{steps}_a{a_in}-{a_fin}_traintime{time_len}_new_{c}.png",
    dpi=300, bbox_inches='tight'
)

plt.savefig(
    f"plots/PCAxcorr_CTCm_ol_N{N}_m{m}_trials{trials}_noisestep{args.noise_steps}_maxnoise{args.noise_max}_a{a}_steps{steps}_a{a_in}-{a_fin}_traintime{time_len}_new_{c}.pdf",
    dpi=300, bbox_inches='tight'
)
plt.show()
...
plt.errorbar(
    aa, mssi_noi_C_avg, yerr=sssi_noi_C_avg,
...
if corr is True:
    ...
    c='uncorrelated'
plt.savefig(
    f"plots/PCAxcorr_Cavg_ol_N{N}_m{m}_trials{trials}_noisestep{args.noise_steps}_maxnoise{args.noise_max}_a{a}_steps{steps}_a{a_in}-{a_fin}_traintime{time_len}_new_{c}.png",
    dpi=300, bbox_inches='tight'
)

plt.savefig(
    f"plots/PCAxcorr_Cavg_ol_N{N}_m{m}_trials{trials}_noisestep{args.noise_steps}_maxnoise{args.noise_max}_a{a}_steps{steps}_a{a_in}-{a_fin}_traintime{time_len}_new_{c}.pdf",
    dpi=300, bbox_inches='tight'
)
plt.show()
