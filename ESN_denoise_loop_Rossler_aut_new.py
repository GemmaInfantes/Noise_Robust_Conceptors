...
xcorro_noi=np.empty((len(k),trials),dtype=float)
xcorro_noi_C_noi=np.empty((len(k),trials),dtype=float)
xcorro_noi_C_ctc=np.empty((len(k),trials),dtype=float)
xcorro_noi_C_ctc_m=np.empty((len(k),trials),dtype=float)

xcorro_noi1=np.empty((len(k),trials),dtype=float)
xcorro_noi_C_noi1=np.empty((len(k),trials),dtype=float)
xcorro_noi_C_ctc1=np.empty((len(k),trials),dtype=float)
...
ph_noi_C_ctc=np.empty((len(k),trials),dtype=float)
...
#             X_noi_C_ctc_ol=forward_rnn(params, ut_train1, seed_noise[j],None,False,C_ctc,std_noise)
...
#             X_noi_C_ctc=forward_rnn_comb(params_trained_CTC, ut_train1, seed_noise[j],steps_ol,None,C_ctc,std_noise)
...
#             xcorro_noi[i,trial_index]=xcorr(y,y_noi,0,steps,flog_min,flog_max,dt)
#             xcorro_noi_C_noi[i,trial_index]=xcorr(y,y_noi_C_noi,0,steps,flog_min,flog_max,dt)
#             xcorro_noi_C_ctc[i,trial_index]=xcorr(y,y_noi_C_ctc,0,steps,flog_min,flog_max,dt)
#             xcorro_noi_C_ctc_m[i,trial_index]=xcorr(y,y_noi_C_ctc_m,0,steps,flog_min,flog_max,dt)
...
#             xcorro_noi_C_ctc1[i,trial_index]=xcorr_new(y,y_noi_C_ctc,0,steps,dt)
...
#             _,ph_noi_C_ctc[i,trial_index]= prediction_horizon(y, y_noi_C_ctc,window,None,0,error_th,steps_th,lyap)
...
# # # ===================== FFT log xcorr boxplots =====================

# plt.figure(figsize=(8, 4), dpi=300)
# positions = np.arange(len(k))
# width = 0.25

# data_noC  = [xcorro_noi[i, :] for i in range(len(k))]
# data_Cnoi = [xcorro_noi_C_noi[i, :] for i in range(len(k))]
# data_Cctc = [xcorro_noi_C_ctc_m[i, :] for i in range(len(k))]
...
# # ===================== SAVE =====================
# plt.savefig(
#     f"plots/FFTxcorr_CTCm_boxplot_trials{trials}_N{N}_m{m}_noisestep{args.noise_steps}_maxnoise{args.noise_max}_a{a}_steps{steps}_anew{a_new}_traintime{time_len}_fmax{flog_max}_fmin{flog_min}.png",
#     dpi=300, bbox_inches="tight"
# )

# plt.savefig(
#     f"plots/FFTxcorr_CTCm_boxplot_trials{trials}_N{N}_m{m}_noisestep{args.noise_steps}_maxnoise{args.noise_max}_a{a}_steps{steps}_anew{a_new}_traintime{time_len}_fmax{flog_max}_fmin{flog_min}.pdf",
#     dpi=300, bbox_inches="tight"
# )
...
#Showing the outputs and FFT for %50 noise
...
seed30=271 #seed (noise,esn): ((1814,1607)) (271,1142)
noi=10
...
Y_noi = X_noi[steps_ol:] @ params_trained_noi['wout'].T + params_trained_noi['bias_out'] #autonomous with noise 
Y_noi_C_ctc = X_noi_C_ctc[steps_ol:] @ params_trained_CTC['wout'].T + params_trained_CTC['bias_out'] #autonomous with noise with ctc C
Y_noi_C_id = X_noi_C_id[steps_ol:] @ params_trained_id['wout'].T + params_trained_id['bias_out'] #autonomous with noise with ctc C
...
xcorro_noi1=xcorr_new(y,y_noi,0,steps,dt)
xcorro_noi_C_ctc1_m=xcorr_new(y,y_noi_C_ctc,0,steps,dt)

print('No C:',xcorro_noi1)
print('CTC:',xcorro_noi_C_ctc1_m)
...
_,ph_noi_C_id30= prediction_horizon(y, y_noi_C_id,window,None,0,error_th,steps_th,lyap)        
...
y_id = np.asarray(Y_noi_C_id).ravel()        
# -------------------------------
# Parameters
# -------------------------------
steps_in = 0
...
# Consistent y-limits for FFT
f=f_target[np.argmax(m_target)]
dif_f=0.04
fft_min = min(m_target[1:].min(), m_noC[1:].min(), m_CTC[1:].min())
fft_max = max(m_target[1:].max(), m_noC[1:].max(), m_CTC[1:].max())
fft_min1 = min(f_target[1:].min(), f_noC[1:].min(), f_CTC[1:].min())
fft_max1 = max(f_target[1:].max(), f_noC[1:].max(), f_CTC[1:].max())

        
fig2, ax2 = plt.subplots(figsize=(8, 4), dpi=300)
ax2.plot(f_target[1:], m_target[1:], '--', color='black', label='Target',alpha=1)
ax2.plot(f_noC[1:], m_noC[1:], '-', color='#B22222', label='Without C',alpha=1,linewidth=2.1)
ax2.plot(f_CTC[1:], m_CTC[1:], '-', color='#1F4E79', label=r'With $C_{ctc}$',alpha=1,linewidth=2.1)
ax2.set_xlabel("Frequency (1/k)")
ax2.set_ylabel("Power Spectrum")
ax2.set_xlim(f-dif_f, f+dif_f)
ax2.set_ylim(fft_min-20, fft_max+30)

# ax2.set_xlim(fft_min1, fft_max1)
