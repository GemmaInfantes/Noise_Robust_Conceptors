    ...
    for k in range(len(noise)): #noise scan    
        for idx in range(trials_esn): #trials of different ESN realizations
            ...
            for j in range(trials_noise): #trials of different noise realizations
                ...
                X_d_C_avg=forward_rnn_drift_new(params, ut_train1, seed_noise[j],None,False,C_avg,std_drift,std_noise)
...
plt.savefig(f"plots/newNRMSE_Cavg_heatmap_N{N}_trials{trials}_noisestep{args.noise_step}_maxnoise{args.noise_max}_bstep{args.b_steps}_maxb{args.b_max}_a{a}_steps{steps}_anew{a_new}_traintime{time_len}_inf{inff}_sup{supf}.png", dpi=300, bbox_inches='tight')
# plt.savefig(f"plots/newNRMSE_Cavg_heatmap_N{N}_trials{trials}_noisestep{args.noise_step}_maxnoise{args.noise_max}_bstep{args.b_steps}_maxb{args.b_max}_a{a}_steps{steps}_anew{a_new}_traintime{time_len}_inf{inff}_sup{supf}.pdf", dpi=300, bbox_inches='tight')
